...
public class FloatView extends RelativeLayout {
    ...
    private FHSurfaceView render_float;
    ...
    public FloatView(Activity context) {
        ...
        LayoutInflater.from(context).inflate(FHResource.getInstance().getId(context,"layout","fh_float_video"),this);
        view = findViewById(FHResource.getInstance().getId(context,"id","rl_float_video"));
        render_float = findViewById(FHResource.getInstance().getId(context,"id","render_float"));
        iv_float = findViewById(FHResource.getInstance().getId(context,"id","iv_float"));
        if(FHUIConstants.isCusIsAudio()){
            render_float.setVisibility(GONE);
            iv_float.setVisibility(VISIBLE);
        }else {
            render_float.setVisibility(VISIBLE);
            iv_float.setVisibility(GONE);
        }
        initLayoutParams();
        initEvent();
    }
    ...
    public void mainEvent(UiEvent ros){
        if(ros != null && ros.getType().endsWith(FHTellerParams.UPT_CUS_STATUS)){
            setVideo(!FHUIConstants.isCusIsAudio());
        }
    }
    public void setVideo(boolean isvideo){
        if(isvideo){
            FHVideoManager.getInstance().changeSurface(render_float);
            render_float.setVisibility(VISIBLE);
            iv_float.setVisibility(GONE);
        }else {
            render_float.setVisibility(GONE);
    ...
    private void initLayoutParams() {
        ...
        layoutParams.x = screenWidth - view.getLayoutParams().width;
        layoutParams.y = 0;
